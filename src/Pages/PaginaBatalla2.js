...
const PaginaUnico = () => {
    ...
    const traerPokemon = ()=>
    {
        const vegeta = db.collection("inscripcionPokemon").doc("bl4LWqUfZkpsUq9Pbt1s")
        vegeta.get().then((doc) => {
            if (doc.exists) {
    ...
    const traerPokemon2 = ()=>
    {
        const vegeta = db.collection("inscripcionPokemon").doc("36ByYZVH9FsnHdbtPcOj")
        vegeta.get().then((doc) => {
            if (doc.exists) {
    ...
    //const pokemon1 = vida<=0?setVida(0):console.log('sigue vivo el primero');
    //const pokemon2 = vida2<=0?setVida2(0):console.log('esta vivo el segundo');
