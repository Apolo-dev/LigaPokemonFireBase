...
const Pagina1Fecha1C = () => {
    ...
    {
        db.collection('GrupoC').where("usuario", "==", usuario.uid).where("bombo", "==", "1").onSnapshot((querySnapshot=>
            {
                const caja = []
    ...
    {
        db.collection('GrupoC').where("usuario", "==", usuario.uid).where("bombo", "==", "4").onSnapshot((querySnapshot=>
            {
                const caja = []
                querySnapshot.forEach((element)=>
                {
    ...
    const handleClickSiguiente = ()=>
    {
        if(vida2 <= 0)
        {
            db.collection('Ganador1GrupoC').add(ganador1)
        }
        else if(vida1 <=0)
        {
            db.collection('Ganador2GrupoC').add(ganador2)
        }
        else
        {
            console.log('siguen');
        ...
        if(vida2 <= 0)
        {
            db.collection('Perdedor1GrupoC').add(perdedor1)
        }
        else if(vida1 <=0)
        {
            db.collection('Perdedor2GrupoC').add(perdedor2)
        }
        else
        {
            console.log('siguen');
        }
        history.push('/fecha2GrupoC')
    }
