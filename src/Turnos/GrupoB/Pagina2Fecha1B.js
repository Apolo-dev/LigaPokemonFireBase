...
const Pagina2Fecha1B = () => {
    ...
    {
        db.collection('GrupoB').where("usuario", "==", usuario.uid).where("bombo", "==", "2").onSnapshot((querySnapshot=>
            {
                const caja = []
    ...
    {
        db.collection('GrupoB').where("usuario", "==", usuario.uid).where("bombo", "==", "3").onSnapshot((querySnapshot=>
            {
                const caja = []
                querySnapshot.forEach((element)=>
                {
    ...
    const handleClickSiguiente = ()=>
    {
        if(vida2 <= 0)
        {
            db.collection('Ganador1GrupoB').add(ganador12)
        }
        else if(vida1 <=0)
        {
            db.collection('Ganador2GrupoB').add(ganador22)
        }
        else
        {
            console.log('siguen');
        ...
        if(vida2 <= 0)
        {
            db.collection('Perdedor1GrupoB').add(perdedor12)
        }
        else if(vida1 <=0)
        {
            db.collection('Perdedor2GrupoB').add(perdedor22)
        }
        else
        {
            console.log('siguen');
        }
        history.push('/resultadosB')
    }
